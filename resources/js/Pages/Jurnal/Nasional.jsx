...
export default function Nasional({ journals, filters, prodiList, sintaList, availableYears, stats }) {
    ...
    const handleReset = () => {
        ...
        setPublishYear('');
        setPerPage('12');
        router.get('/jurnal-nasional', {}, { preserveState: true });
    };
    ...
    return (
        <div className="min-h-screen bg-[#FDFBF7] text-slate-900 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
            <Head title="Direktori Jurnal Nasional SINTA - Politeknik Indonusa Surakarta" />
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <div className="flex items-center space-x-1.5">
                                <span className="font-extrabold text-slate-900 text-lg tracking-tight">Jurnal Ilmiah Nasional</span>
                                <span className="px-2 py-0.5 text-[10px] font-extrabold bg-blue-600 text-white rounded-full">
                                    {stats?.total || 0} Jurnal
                                </span>
                            </div>
                            <p className="text-xs text-slate-500 font-medium">Pangkalan direktori jurnal nasional terakreditasi SINTA & ber-ISSN Politeknik Indonusa</p>
                        </div>
                    </div>
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-3">
                        ...
                        <div>
                            <p className="text-xs text-slate-500 font-semibold">SINTA 1 - 3</p>
                            <p className="text-lg font-black text-slate-900">{stats?.sinta_top || 0}</p>
                        </div>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-3">
                        ...
                        <div>
                            <p className="text-xs text-slate-500 font-semibold">Terakreditasi SINTA</p>
                            <p className="text-lg font-black text-slate-900">{stats?.sinta_all || 0}</p>
                        </div>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-3">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xs font-extrabold text-slate-900 uppercase tracking-wider flex items-center space-x-2">
                            ...
                            <span>Pencarian Jurnal Nasional (SINTA)</span>
                        </h2>
                    </div>
                    ...
                    <form onSubmit={handleFilter} className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                        <div className="sm:col-span-5 relative">
                            ...
                            <input
                                ...
                                placeholder="Ketik judul jurnal, penerbit, program studi, atau ISSN..."
                                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-300 rounded-2xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 text-xs font-medium"
                            />
                        </div>
                        ...
                        <div className="sm:col-span-3">
                            <select
                                ...
                                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-300 rounded-2xl text-slate-900 focus:outline-none focus:border-blue-500 text-xs font-medium cursor-pointer"
                            >
                        ...
                        <div className="sm:col-span-2 flex space-x-2">
                            <button
                                type="submit"
                                className="flex-1 py-2.5 px-3 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl text-xs font-bold shadow flex items-center justify-center space-x-1 cursor-pointer transition-all"
                            >
                ...
                {journals.data.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {journals.data.map((journal) => (
                            <div
                                key={journal.id}
                                className="bg-white rounded-3xl p-6 border border-slate-200/80 hover:border-blue-400 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between space-y-4 group"
                            >
                                <div className="space-y-3.5">
                                    {/* Badges */}
                                    <div className="flex items-center justify-between gap-1.5">
                                        <span className="px-2.5 py-1 bg-blue-100/90 text-blue-900 border border-blue-200 font-extrabold rounded-full text-[10px]">
                                            Jurnal Nasional
                                        </span>
                                        {journal.sinta && (
                                            <span className="px-2.5 py-0.5 font-bold rounded-full text-[10px] bg-amber-50 text-amber-900 border border-amber-300 flex items-center space-x-1">
                                    ...
                                    <div className="w-full h-36 rounded-2xl bg-gradient-to-br from-blue-900 via-slate-900 to-indigo-950 overflow-hidden relative shadow-sm group-hover:scale-[1.02] transition-transform flex flex-col justify-between p-4 text-white">
                                        {journal.cover_image ? (
                                            <img
                                            ...
                                            <>
                                                <div className="flex items-center justify-between text-blue-300">
                                                    ...
                                                    <span className="text-[10px] font-mono font-bold">{journal.sinta || 'NASIONAL'}</span>
                                                </div>
                                                ...
                                                <p className="text-[10px] font-medium text-slate-300 line-clamp-1">
                                                    {journal.prodi || 'Politeknik Indonusa Surakarta'}
                                                </p>
                                            </>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        <h3
                                            ...
                                            className="font-extrabold text-base text-slate-950 hover:text-blue-600 transition-colors line-clamp-2 leading-snug cursor-pointer"
                                        >
                                            {journal.title}
                                        </h3>
                                ...
                                <div className="pt-3 border-t border-slate-200/80 flex items-center justify-between gap-2">
                                    ...
                                    {journal.access_url ? (
                                        <a
                                            ...
                                            className="px-4 py-1.5 bg-slate-950 hover:bg-blue-600 text-white font-bold text-xs rounded-xl shadow-sm transition-all flex items-center space-x-1.5 shrink-0"
                                        >
                    ...
                    <div className="bg-white rounded-3xl p-12 text-center border border-slate-200/80 space-y-4">
                        ...
                        <h3 className="text-base font-extrabold text-slate-950">Tidak ada jurnal nasional ditemukan</h3>
                        <p className="text-xs text-slate-500 max-w-sm mx-auto">
                            Coba ubah kata kunci pencarian atau reset filter untuk melihat seluruh katalog jurnal nasional.
                        </p>
                        <button
                            type="button"
                            onClick={handleReset}
                            className="px-5 py-2.5 bg-slate-950 text-white font-bold text-xs rounded-full hover:bg-blue-600 transition-all cursor-pointer"
                        >
                            Reset Semua Filter
                        </button>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100 p-6 sm:p-8 space-y-6">
                        <div className="flex items-start justify-between">
                            <div className="space-y-1">
                                <span className="px-2.5 py-0.5 bg-blue-100 text-blue-900 border border-blue-200 font-extrabold rounded-full text-[10px]">
                                    Jurnal Nasional
                                </span>
                        ...
                        <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl text-xs">
                            ...
                            <div>
                                <span className="text-slate-400 block font-semibold">Akreditasi SINTA:</span>
                        ...
                        <div className="pt-4 border-t border-slate-100 flex items-center justify-end space-x-3">
                            ...
                            {selectedJournal.access_url && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-2xl shadow-md transition-all flex items-center space-x-2"
                                >
            ...
            <footer className="bg-slate-950 text-slate-400 py-10 border-t border-slate-800 mt-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between text-xs space-y-3 sm:space-y-0">
                    <p>© {new Date().getFullYear()} UPT Perpustakaan Politeknik Indonusa Surakarta. Pangkalan Data Jurnal Nasional SINTA.</p>
